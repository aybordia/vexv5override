...
pros::MotorGroup dr4b_motors({7, -8}, pros::MotorGears::red);
...
constexpr double DR4B_MIN_POSITION = 0;   // degrees, lift resting at the bottom
constexpr double DR4B_MAX_POSITION = 900; // degrees, lift fully raised
constexpr int DR4B_VELOCITY = 100;        // rpm, red cartridge tops out at 100

bool dr4b_raised = false; // true while the lift is commanded to the top

void dr4b_toggle() {
    dr4b_raised = !dr4b_raised;
    dr4b_motors.move_absolute(dr4b_raised ? DR4B_MAX_POSITION : DR4B_MIN_POSITION, DR4B_VELOCITY);
}
...
void initialize() {
    ...
    // Treat wherever the lift is sitting at startup as the bottom of its
    // travel, so the absolute moves above line up with the real mechanism.
    dr4b_motors.tare_position();
    // Hold position against gravity instead of coasting back down.
    dr4b_motors.set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);
}
...
void opcontrol() {
    while (true) {
        // --- Drive: arcade, left stick forward/back, right stick turn -------
        int leftY = controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_Y);
        int rightX = controller.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_X);
        chassis.arcade(leftY, rightX);

        // --- DR4B: L1 toggles between the bottom and the top ---------------
        if (controller.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_L1)) dr4b_toggle();

        // --- Claw: R1 opens it, R2 closes it -------------------------------
        if (controller.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_R1)) claw.extend();
        if (controller.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_R2)) claw.retract();
...
// ===========================================================================
// LAST SEASON'S CODE - commented out, kept for reference only.
// ===========================================================================
// #include "main.h"
// #include "lemlib/api.hpp"
// #include "pros/misc.h"
